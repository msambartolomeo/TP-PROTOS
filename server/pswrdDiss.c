...
static enum pop3State pop3_parse_command(struct pop3_parser *parser, uint8_t byte, enum pop3State state) {
    if (byte == '\n' && parser->remaining == 5) {
...
static enum pop3State pop3_parse_argument(struct pop3_parser *parser, uint8_t byte, enum pop3State state) {
    if (byte == '\r' || byte == '\n' || parser->remaining == 0) {
        *parser->current = 0;
