...
static struct ip_and_port * sockaddr_to_human(struct ip_and_port * ip_port,
    ...
    if (inet_ntop(addr->sa_family, p, ip_port->ip, INET6_ADDRSTRLEN + 1) ==
        NULL) {
        return NULL;
    }
...
void logger(enum log_type type, socks5_connection * conn) {
    ...
    case ADDRESS_TYPE_IPV4:
        destination = sockaddr_to_human(&destination_ip_port,
                                        (struct sockaddr *)&conn->parser.request
    ...
    switch (type) {
    case LOG_ACCESS:
        sockaddr_to_human(&source_ip_port,
                          (struct sockaddr *)&conn->client_addr);
        printf("%s\t%s\t%c\t%s\t%hu\t%s\t%hu\t%d\n", time_str, user, type,
               source_ip_port.ip, htons(source_ip_port.port), destination,
               ntohs(conn->parser.request.request.port),
               conn->parser.request.response.status);
        break;
    case LOG_PASSWORD:
        printf("%s\t%s\t%c\tPOP3\t%s\t%hu\t%s\t%s\n", time_str, user, type,
               destination, ntohs(conn->parser.request.request.port),
               conn->pop3.info.user, conn->pop3.info.pass);
        break;
    default:
        return;
    }
