...
static enum request_state addr_type(struct request_parser * parser,
    ...
    case ADDRESS_TYPE_IPV4:
        parser->remaining = 4;
        memset(&(parser->request.destination.ipv4), 0,
               sizeof(parser->request.destination.ipv4));
        parser->request.destination.ipv4.sin_family = AF_INET;
        parser->pointer =
            (uint8_t *)&(parser->request.destination.ipv4.sin_addr);
        return REQUEST_DST_ADDRESS;
    case ADDRESS_TYPE_IPV6:
        parser->remaining = 16;
        memset(&(parser->request.destination.ipv6), 0,
...
static void request_parse_byte(struct request_parser * parser, uint8_t byte) {
    ...
    case REQUEST_RESERVED:
        parser->state =
            byte == 0x00 ? REQUEST_ADDRESS_TYPE : REQUEST_ERROR_MISSING_RSV;
        break;
